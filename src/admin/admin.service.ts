...
export class AdminService {
  ...
  async registration(createAdminDto: CreateAdminDto, res: Response) {
    ...
    console.log(createAdminDto.confirm_password);
    console.log(createAdminDto.password);
  ...
  async getByParam(findAdminDto: FindAdminDto) {
    const params = { ...findAdminDto };
    const where: any = {};
    if (params.name) {
      where.name = { [Op.like]: `%${params.name}%` };
    }
    const admin = await this.adminRepo.findAll({ where: where });
    return admin;
  }
}
